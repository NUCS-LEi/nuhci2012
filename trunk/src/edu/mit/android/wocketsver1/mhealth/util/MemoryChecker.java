package edu.mit.android.wocketsver1.mhealth.util;
...
public class MemoryChecker {
	...
	public static void alertIfMemoryLow(Context aContext)
	{
		...
		{
			msg = "The internal memory on the phone is too low (" + getDataDirectoryStorageAvailable() + "). Please delete some programs so MHealth can run properly!";
		}
		if (isExternalMemoryLow())
		{
			msg += "The SD card is running out of space (" + getExternalStorageAvailable() + "). Please delete some pictures/videos so MHealth can run properly!";
		}		
	...
	public static double getExternalStorageAvailable() {
		return (getAvailableStorageForPath(Environment.getExternalStorageDirectory().getPath())/1000/1000);
	}
	...
	public static double getDataDirectoryStorageAvailable() {
		return (getAvailableStorageForPath(Environment.getDataDirectory().getPath())/1000/1000);
	}	
}
